...
The main results reported here are `normals' in a modified meteorological sense of term.
For a variable of interest (eg. temperature), 
...
are computed for each calendar day.
Figure \ref{fig:main_summary} shows the minimum and maximum of the normals for temperatures
along with the mean of the normal quarantine length predictions based on 3 generation degree day accumulation 
and MED-FOES 95\% extirpation.
Figures \ref{fig:DD_variation_summary} and \ref{fig:pe95_variation_summary} show the 
standard deviations ($\sigma$) of the normals for the degree day and MED-FOES based quarantine length 
predictions.
...
The results reported here are the normals of predicted quarantine durations computed using the actual 
temperature time series,
not the predicted quarantine durations computed from a normal of the temperature time series.
